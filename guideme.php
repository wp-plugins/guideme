...
class GuideMe {
	...
	function wp_dropdown_pages_gm($args = '') {
		...
		extract( $r, EXTR_SKIP );
		$pages = new WP_Query(array('posts_per_page' => -1, 'no_found_rows' => true, 'post_status' => 'publish', 'ignore_sticky_posts' => true, 'post_type' => $r['post_type'], 'post__not_in' => $this->remove_pages_from_select() ));
		$pages = $pages->posts;
		$output = '';
		if ( empty($id) )
			$id = $name;
...
/*
add_filter( 'gm_remove_terms_from_select', 'remove_terms_from_select' );
function remove_terms_from_select($terms){
	$terms[] = 15;
	return $terms;
}

add_filter( 'gm_remove_posttype_from_select', 'remove_posttype_from_select' );
function remove_posttype_from_select($posttype){
	$posttype[] = 'page';
	return $posttype;
}

add_filter( 'gm_remove_taxonomy_from_select', 'remove_taxonomy_from_select' );
function remove_taxonomy_from_select($taxonomy){
	$taxonomy[] = 'post_tag';
	return $taxonomy;
}
*/
